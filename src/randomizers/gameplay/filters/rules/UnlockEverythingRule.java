...
public class UnlockEverythingRule implements Rule {
  ...
  private static final ImmutableSet<String> DO_NOT_TOUCH = ImmutableSet.of("Door.Door_LevelTransition_AlexBunker");

  private static final ImmutableMap<String, String> DESIRED_STATE = new ImmutableMap.Builder<String, String>().put(
      "bStartsUnlocked", "1")
      .put("bStartsLocked", "0")
      .put("bLocked", "0")
      .put("bStartsPoweredOn", "1")
      .put("bUseFreeExitButton", "1")
      .put("bStartsBroken", "0")
      .build();
  ...
  public boolean trigger(Element e, Random r, String filename) {
    // Check if Properties2 child exists and contains one of the desired state keys
    // with a
    // mismatched value.
    ...
    for (String key : DESIRED_STATE.keySet()) {
      if (properties.getAttributeValue(key) != null && !properties.getAttributeValue(key)
          .equals(DESIRED_STATE.get(key))) {
        return true;
      }
    }
    return false;
